            ...
            Int_t scan)
{
  ...
  if (fit_type == 0) {
    fit_model_x = new TF1("fit_model", fit_GP2, -1, 1, Get_number_par(fit_type));
  } else if (fit_type == 1) {
    fit_model_x = new TF1("fit_model", fit_GP6, -1, 1, Get_number_par(fit_type));
  } else if (fit_type == 2) {
    fit_model_x = new TF1("fit_model", fit_G, -1, 1, Get_number_par(fit_type));
  } else if (fit_type == 4) {
    fit_model_x = new TF1("fit_model", fit_DG, -1, 1, Get_number_par(fit_type));
  } else {
  ...
  if (fit_type == 0) {
    fit_model_y = new TF1("fit_model", fit_GP2, -1, 1, Get_number_par(fit_type));
  } else if (fit_type == 1) {
    fit_model_y = new TF1("fit_model", fit_GP6, -1, 1, Get_number_par(fit_type));
  } else if (fit_type == 2) {
    fit_model_y = new TF1("fit_model", fit_G, -1, 1, Get_number_par(fit_type));
  } else if (fit_type == 4) {
    fit_model_y = new TF1("fit_model", fit_DG, -1, 1, Get_number_par(fit_type));
  } else {
    cout << " Fit model " << fit_type << " not known " << endl;
    exit(-105);
  }
  ...
  for (Int_t i = 0; i < nIBC; i++) {
    ...
    if (chi2_dof_x > 0)
      bcc_x_H->SetBinContent(i + 1, (fit_model_x->Eval(par_x[1])) / (fit_model_x->Eval(0)));
    if (chi2_dof_y > 0)
      bcc_y_H->SetBinContent(i + 1, (fit_model_y->Eval(par_y[1])) / (fit_model_y->Eval(0)));
  }

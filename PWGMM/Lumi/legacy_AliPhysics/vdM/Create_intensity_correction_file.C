...
// 3.- loop over the range of each step and get the correction and its error
{
  ...
  if (scan_type == 1) {
    if (intensity_type == 1) {
      sprintf(file_name, "../Fill-%d/BPTXCorr_x_Scan_%d.root", g_vdm_Fill, scan);
    } else if (intensity_type == 2) {
      sprintf(file_name, "../Fill-%d/FBCTCorr_x_Scan_%d.root", g_vdm_Fill, scan);
    }
  } else if (scan_type == 2) {
    if (intensity_type == 1) {
      sprintf(file_name, "../Fill-%d/BPTXCorr_y_Scan_%d.root", g_vdm_Fill, scan);
    } else if (intensity_type == 2) {
      sprintf(file_name, "../Fill-%d/FBCTCorr_y_Scan_%d.root", g_vdm_Fill, scan);
    }
  }

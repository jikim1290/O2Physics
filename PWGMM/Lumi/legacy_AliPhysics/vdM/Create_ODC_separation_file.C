...
Double_t Get_ODC_x_scan_6864(Double_t mid_time)
{
  ...
  Double_t x = (mid_time - p0);
  // divide by 1000 to go from mum to mm
  return (p1 + p2 * x + p3 * x * x + p4 * x * x * x + p5 * x * x * x * x) / 1000;
}
...
// scan_type: 1 => x-scan; 2 => y-scan
{
  ...
  if (scan_type == 1)
    sprintf(sep_file_name, "../Fill-%d/NomSep_x_Scan_%d.root", g_vdm_Fill, scan);
  if (scan_type == 2)
    sprintf(sep_file_name, "../Fill-%d/NomSep_y_Scan_%d.root", g_vdm_Fill, scan);
  ...
  if (scan_type == 1)
    sprintf(sep_file_name, "../Fill-%d/ODCSep_x_Scan_%d.root", g_vdm_Fill, scan);
  if (scan_type == 2)
    sprintf(sep_file_name, "../Fill-%d/ODCSep_y_Scan_%d.root", g_vdm_Fill, scan);

...
void GetSeparation(Double_t* sep, const char* separation_name, Int_t scan, Int_t scan_type, Int_t bc)
{
  ...
  if (scan_type == 1)
    sprintf(file_name, "../Fill-%d/%sSep_x_Scan_%d.root", g_vdm_Fill, separation_name, scan);
  if (scan_type == 2)
    sprintf(file_name, "../Fill-%d/%sSep_y_Scan_%d.root", g_vdm_Fill, separation_name, scan);

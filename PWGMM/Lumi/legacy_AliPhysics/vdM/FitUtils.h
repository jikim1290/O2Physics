...
// returns chi2/dof ... returns -1 if fit does not converge
{
  ...
  TF1* fit_model = NULL;
  if (fit_type == 0) {
    fit_model = new TF1("fit_model", fit_GP2, sep_min, sep_max, Get_number_par(fit_type));
  } else if (fit_type == 1) {
    fit_model = new TF1("fit_model", fit_GP6, sep_min, sep_max, Get_number_par(fit_type));
  } else if (fit_type == 2) {
    fit_model = new TF1("fit_model", fit_G, sep_min, sep_max, Get_number_par(fit_type));
  } else if (fit_type == 4) {
    fit_model = new TF1("fit_model", fit_DG, sep_min, sep_max, Get_number_par(fit_type));
  } else {
    cout << " Fit model " << fit_type << " not known " << endl;
    exit(-105);
  }
  ...
  if (!gMinuit->fCstatu.Contains("CONVERGED") ||
    ...
    TH1F* H1Temp = new TH1F(Form("%s_temp", dName.c_str()), "", n, sep[0], sep[n - 1]);
    H1Temp->Sumw2();
    for (int a = 0; a < n; a++) {

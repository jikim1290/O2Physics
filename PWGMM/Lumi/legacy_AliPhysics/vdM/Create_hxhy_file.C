...
// then loop over all bunch crossings and for each of them make a fit
{
  ...
  if (scan_type == 1)
    sprintf(sep_file_name, "../Fill-%d/%sSep_x_Scan_%d.root", g_vdm_Fill, sep_type, scan);
  if (scan_type == 2)
    sprintf(sep_file_name, "../Fill-%d/%sSep_y_Scan_%d.root", g_vdm_Fill, sep_type, scan);

...
void GetRate(Double_t* rate, const char* rate_name, const char* rate_type, Int_t scan, Int_t scan_type, Int_t bc)
{
  ...
  if (scan_type == 1)
    sprintf(file_name, "../Fill-%d/%sRate_%s_x_Scan_%d.root", g_vdm_Fill, rate_type, rate_name, scan);
  if (scan_type == 2)
    sprintf(file_name, "../Fill-%d/%sRate_%s_y_Scan_%d.root", g_vdm_Fill, rate_type, rate_name, scan);
...
// scan_type: 1 => x-scan; 2 => y-scan
{
  ...
  if (doOptical)
    GetRate(optical, rate_name, "OpticalIntensityCorrFBCT", scan, scan_type, bc);
  ...
  for (Int_t i = 0; i < n_sep; i++) {
    ...
    if (pu[i] > 0 && all[i] > 0)
      ...
      pu_all[i] = -1;
    if (doOptical && optical[i] > 0 && all[i] > 0)
      all_optical[i] = all[i] / optical[i];
    else
      all_optical[i] = -1;
  }
  ...
  for (Int_t i = 0; i < n_sep; i++) {
    if (raw_bkgd[i] > -1 && raw_bkgd[i] > ratio_max)
      ratio_max = raw_bkgd[i];
    if (bkgd_pu[i] > -1 && bkgd_pu[i] > ratio_max)
      ratio_max = bkgd_pu[i];
    if (pu_all[i] > -1 && pu_all[i] > ratio_max)
      ratio_max = pu_all[i];
    if (doOptical && all_optical[i] > -1 && all_optical[i] > ratio_max)
      ratio_max = all_optical[i];
    if (raw_bkgd[i] > -1 && raw_bkgd[i] < ratio_min)
      ratio_min = raw_bkgd[i];
    if (bkgd_pu[i] > -1 && bkgd_pu[i] < ratio_min)
      ratio_min = bkgd_pu[i];
    if (pu_all[i] > -1 && pu_all[i] < ratio_min)
      ratio_min = pu_all[i];
    if (doOptical && all_optical[i] > -1 && all_optical[i] < ratio_min)
      ratio_min = all_optical[i];
  }
  ratio_max *= 1.5;
  ratio_min *= 0.8;
  ...
  if (doOptical) {
    ...
    gr_optical_all->SetMarkerColor(3);
  }
  ...
  if (doOptical)
    legend->AddEntry(gr_optical_all, "Intensity/Optical", "p");
  legend->Draw();

...
// scan_type: 1 => x-scan; 2 => y-scan
{
  ...
  // --> separation file
  if (scan_type == 1)
    sprintf(file_name, "../Fill-%d/%sSep_x_Scan_%d.root", g_vdm_Fill, sep_type, scan);
  if (scan_type == 2)
    sprintf(file_name, "../Fill-%d/%sSep_y_Scan_%d.root", g_vdm_Fill, sep_type, scan);
  ...
  Double_t rate_max = 0;
  for (Int_t i = 0; i < n_sep; i++) {
    if ((rate[i] + rate_error[i]) > rate_max)
      rate_max = rate[i] + rate_error[i];
  }

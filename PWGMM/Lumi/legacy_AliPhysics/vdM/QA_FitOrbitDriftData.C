...
void ProcessFill6012(Int_t Fill)
{
  ...
  for (Int_t i = 0; i < gr_hor->GetN(); i++) {
    ...
    if (TMath::IsNaN(drift_hor))
      continue;
    if (TMath::IsNaN(drift_ver))
      continue;
    ...
    if (time > max_time)
      max_time = time;
    if (drift_hor < min_drift)
      min_drift = drift_hor;
    if (drift_hor > max_drift)
      max_drift = drift_hor;
    if (drift_ver < min_drift)
      min_drift = drift_ver;
    if (drift_ver > max_drift)
      max_drift = drift_ver;
    // apply the selection
    if (timeL > time_start_x0 && timeL < time_end_x0)
      continue;
    if (timeL > time_start_y0 && timeL < time_end_y0)
      continue;
    if (timeL > time_start_x1 && timeL < time_end_x1)
      continue;
    if (timeL > time_start_y1 && timeL < time_end_y1)
      continue;
    if (timeL > (time_end_y1 + 200))
      continue;
    // fill new graph
  ...
  gr_hor->SetMarkerColor(2);
  gr_hor->SetMarkerSize(0.2);
  gr_hor->Draw("p,same");
  gr_ver->SetMarkerStyle(20);
  gr_ver->SetMarkerColor(4);
  gr_ver->SetMarkerSize(0.2);
  gr_ver->Draw("p,same");
  // selected data
  gr_hor_new->SetMarkerStyle(20);
  gr_hor_new->SetMarkerColor(2);
  gr_hor_new->Draw("p,same");
  gr_ver_new->SetMarkerStyle(20);
  gr_ver_new->SetMarkerColor(4);

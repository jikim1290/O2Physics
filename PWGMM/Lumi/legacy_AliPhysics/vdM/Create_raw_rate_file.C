...
// 2.- loop over the range of each step and get the raw rate and its error
{
  ...
  if (scan_type == 1)
    sprintf(file_name, "../Fill-%d/RawRate_%s_x_Scan_%d.root", g_vdm_Fill, rate_name, scan);
  if (scan_type == 2)
    sprintf(file_name, "../Fill-%d/RawRate_%s_y_Scan_%d.root", g_vdm_Fill, rate_name, scan);
  ...
  {
    if (k % 10 == 0) {
      cout << " Scan " << scan << "," << scan_type << " working on bunch " << k << " of " << nIBC << endl
           << flush;
    }

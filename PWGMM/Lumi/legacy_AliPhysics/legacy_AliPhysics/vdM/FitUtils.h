...
Int_t n_minuit;
Double_t x_minuit[100]; // 100 is a 'large' number, normally there are only 25
                        // entries per scan
...
Int_t Get_number_par(Int_t fit_type)
{
  if (fit_type == 0)
    ...
    return 1; // NUM, this will be dummy parameters
  else if (fit_type == 4)
    return 5; // DG
  else
    return -1;
                             ...
                             Double_t* par, Int_t iflag)
{
  ...
  for (Int_t i = 0; i < n_minuit; i++) {
    ...
    Double_t f_i = 0;
    if (fit_type_minuit == 0)
      f_i = fit_GP2(&x_minuit[i], par);
    else if (fit_type_minuit == 1)
      f_i = fit_GP6(&x_minuit[i], par);
    else if (fit_type_minuit == 2)
      f_i = fit_G(&x_minuit[i], par);
    // compute the likelihood for this term
    if (nll_model == 0) { // gaussian model
      if (y_minuit[i] < 1e-6 || ye_minuit[i] < 1e-6)
        continue; // avoid 'zero' rates
...
/*
void negative_log_likelihood(Int_t &npar, Double_t *gin, Double_t &nll, Double_t
*par, Int_t iflag)
{
 Double_t sum = 0;
 for (Int_t i = 0; i < n_minuit; i++) {
...
// (scan, scan_type and bc are only used to give a proper name to canvas if
// necessary)
{
  // if fit_type = 3, do numeric integration
  if (fit_type == 3)
    return Do_Numeric_Integration(n, sep, rate, rate_err, area, rate_zero, par,
                                  par_err);
  ...
  m.DefineParameter(0, "R", scale_minuit * rate_max, 0.01,
                    scale_minuit * 0.5 * rate_max,
                    scale_minuit * 2.0 * rate_max);
  ...
  for (Int_t i = 0; i < nPar; i++)
    m.GetParameter(i, par[i], par_err[i]);
  ...
  if (bc > -1) {
    ...
    fit_model->Draw("same");
    cout << name << " A= " << area[0] << "+/-" << area[1]
         << " R0= " << rate_zero[0] << "+/-" << rate_zero[1] << " chi2 " << chi2
         << endl;
    if (!m.fCstatu.Contains("CONVERGED"))
...
// returns chi2/dof ... returns -1 if fit does not converge
{
  ...
  if (fit_type == 0) {
    ...
    fit_model = new TF1("fit_model", fit_GP6, sep_min, sep_max,
                        Get_number_par(fit_type));
  } else if (fit_type == 2) {
    fit_model =
  ...
  if (!gMinuit->fCstatu.Contains("CONVERGED") || (fitQuality < fitQuality_TL) ||
    ...
    if (reFit == reFit_max) {
      cout << Form(" Fit failed: %s, chi2/NDF: %4.3f", dName.c_str(),
                   fitQuality)
           << endl;
      dName = Form("%s_FAIL", dName.c_str());
      fitQuality = -1;
    } else
      dName = Form("%s_reFit%i", dName.c_str(), reFit);
    H1Temp->Delete();
  } // Retry fit

  /*
TCanvas *c = new TCanvas(dName.c_str(), dName.c_str(), 800, 600); c->cd();
TH1 *h = (TH1*)gr->GetHistogram(); h->SetTitle(Form("%s;Separation (mm); Rate
(Hz)", dName.c_str())); h->Draw(); gr->SetMarkerStyle(20); gr->Draw("pe same");
fit_model->Draw("same");
...
//c->Print(Form("../Fill-%d/QA_fits/%s.png", g_vdm_Fill, dName.c_str()));
  ...
  */
  ...
  area[0] = fit_model->Integral(sep_min, sep_max, epsilon);
  area[1] = fit_model->IntegralError(sep_min, sep_max, r->GetParams(),

...
//-------------------------------------------------------
// Create a background corrected rate file for a given
// trigger, background correction and scan
//-------------------------------------------------------

void Create_one_bkgd_corrected_rate_file(Int_t scan_type, Int_t scan,
                                         const char* rate_name,
                                         const char* bkgd_corr_name)
// scan_type: 1 => x-scan; 2 => y-scan
{
  ...
  if (scan_type == 1) {
    sprintf(file_name_rate, "../Fill-%d/RawRate_%s_x_Scan_%d.root", g_vdm_Fill,
            rate_name, scan);
    sprintf(file_name_corr, "../Fill-%d/BkgdCorr_%s_x_Scan_%d.root", g_vdm_Fill,
            bkgd_corr_name, scan);
    sprintf(file_name_new_rate, "../Fill-%d/BkgdCorrRate_%s_x_Scan_%d.root",
            g_vdm_Fill, rate_name, scan);
  }
  if (scan_type == 2) {
    sprintf(file_name_rate, "../Fill-%d/RawRate_%s_y_Scan_%d.root", g_vdm_Fill,
            rate_name, scan);
    sprintf(file_name_corr, "../Fill-%d/BkgdCorr_%s_y_Scan_%d.root", g_vdm_Fill,
            bkgd_corr_name, scan);
    sprintf(file_name_new_rate, "../Fill-%d/BkgdCorrRate_%s_y_Scan_%d.root",
            g_vdm_Fill, rate_name, scan);
  }
  ...
  TTree* corr_tree = (TTree*)CorrFile->Get("BkgdCorr");
  ...
  corr_tree->SetBranchAddress("correction", correction);
  corr_tree->SetBranchAddress("correction_error", correction_error);
  ...
  for (Int_t k = 0; k < nIBC; k++) {
    ...
    {
      new_rate[i] = rate[i] * correction[i];
      new_rate_error[i] = BkgdCorrectedRateError(
        rate[i], rate_error[i], correction[i], correction_error[i]);
    }
...
void Create_bkgd_corrected_rate_file(Int_t Fill, const char* rate_name,
                                     const char* bkgd_corr_name)
{
  ...
  for (Int_t i = 0; i < g_n_Scans_in_Fill; i++) {
    Create_one_bkgd_corrected_rate_file(1, i, rate_name,
                                        bkgd_corr_name); // x-scans
    Create_one_bkgd_corrected_rate_file(2, i, rate_name,
                                        bkgd_corr_name); // y-scans
  }

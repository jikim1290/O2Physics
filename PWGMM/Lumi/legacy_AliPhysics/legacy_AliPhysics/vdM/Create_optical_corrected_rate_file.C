...
//   using the correction based on the ODC rates (ie directory FBCT, not
...
// "+Q", "+xi", "-Q", "-xi"
{
  ...
  if (strcmp(rate_name, "TVX") == 0)
    ...
    sprintf(optical_file_name,
            "../Fill-%d/Corr-%d-sys/FBCT/ROOT/OPT-bbroot_%d_V0%s.root",
            g_vdm_Fill, g_vdm_Fill, g_vdm_Fill, sys_opt);
...
sprintf(file_name_rate_y,"../Fill-%d/IntensityCorrFBCTRate_%s_y_Scan_%d.root",
    ...
    // Modified by kimc
    sprintf(file_name_new_rate_x,
            "../Fill-%d/OpticalIntensity%sCorr%sRate_%s_x_Scan_%d.root",
            g_vdm_Fill, sys_opt, intensity_name, rate_name, scan);
    sprintf(file_name_new_rate_y,
            "../Fill-%d/OpticalIntensity%sCorr%sRate_%s_y_Scan_%d.root",
            g_vdm_Fill, sys_opt, intensity_name, rate_name, scan);
    sprintf(file_name_rate_x,
            "../Fill-%d/IntensityCorr%sRate_%s_x_Scan_%d.root", g_vdm_Fill,
            intensity_name, rate_name, scan);
    sprintf(file_name_rate_y,

...
Double_t Get_ODC_x_scan_6864(Double_t mid_time)
{
  ...
  Double_t p3 = 1.86668e-06;
  Double_t p4 = -9.50342e-10;
  Double_t p5 = 1.0986e-13;
  Double_t x = (mid_time - p0);
  // divide by 1000 to go from mum to mm
  return (p1 + p2 * x + p3 * x * x + p4 * x * x * x + p5 * x * x * x * x) /
         1000;
}
...
Double_t Get_ODC_separations_Fill(Double_t mid_time, Int_t scan_type,
                                  Int_t scan)
// scan_type: 1 => x-scan; 2 => y-scan
{
  if (g_vdm_Fill == 4690) {
    if (scan_type == 1)
      return Get_ODC_x_scan_4690(mid_time);
    if (scan_type == 2)
      return Get_ODC_y_scan_4690(mid_time);
    return 0;
  } else if (g_vdm_Fill == 4937) {
    if (scan_type == 1)
      return Get_ODC_x_scan_4937(mid_time);
    if (scan_type == 2)
      return Get_ODC_y_scan_4937(mid_time);
    return 0;
  } else if (g_vdm_Fill == 5533) {
    if (scan_type == 1)
      return Get_ODC_x_scan_5533(mid_time);
    if (scan_type == 2)
      return Get_ODC_y_scan_5533(mid_time);
    return 0;
  } else if (g_vdm_Fill == 5568) {
    if (scan_type == 1)
      return Get_ODC_x_scan_5568(mid_time);
    if (scan_type == 2)
      return Get_ODC_y_scan_5568(mid_time);
    return 0;
  } else if (g_vdm_Fill == 6012) {
    if (scan_type == 1)
      return Get_ODC_x_scan_6012(mid_time);
    if (scan_type == 2)
      return Get_ODC_y_scan_6012(mid_time);
    return 0;
  } else if (g_vdm_Fill == 6864) {
    if (scan_type == 1)
      return Get_ODC_x_scan_6864(mid_time);
    if (scan_type == 2)
      return Get_ODC_y_scan_6864(mid_time);
    return 0;
  } else if (g_vdm_Fill == 7483) {
    if (scan_type == 1 && scan == 0)
      return Get_ODC_x_scan0_7483(mid_time);
    if (scan_type == 1 && scan == 1)
      return Get_ODC_x_scan1_7483(mid_time);
    if (scan_type == 2 && scan == 0)
      return Get_ODC_y_scan0_7483(mid_time);
    if (scan_type == 2 && scan == 1)
      return Get_ODC_y_scan1_7483(mid_time);
    return 0;
  } else {
...
// scan_type: 1 => x-scan; 2 => y-scan
{
  ...
  if (scan_type == 1)
    sprintf(sep_file_name, "../Fill-%d/NomSep_x_Scan_%d.root", g_vdm_Fill,
            scan);
  if (scan_type == 2)
    sprintf(sep_file_name, "../Fill-%d/NomSep_y_Scan_%d.root", g_vdm_Fill,
            scan);
  ...
  for (Int_t i = 0; i < n_sep; i++) {
    ...
    // cout << " i " << i << " end " << time_end << " start " << time_start <<
    // endl; cout << " mid_time " << ((Long_t) mid_time) << " corr " << corr <<
    // " odc_sep " << odc_sep[i] << " sep " << sep[i] << endl;
  }

  // now prepare name of output and reserve space to store it
  if (scan_type == 1)
    sprintf(sep_file_name, "../Fill-%d/ODCSep_x_Scan_%d.root", g_vdm_Fill,
            scan);
  if (scan_type == 2)
  ...
  // (the info is repeated, because it for ODC separation it is the same for all
  // bunches)
  // -- number of bc
...
void Create_ODC_separation_file(Int_t Fill)
{
  ...
  for (Int_t i = 0; i < g_n_Scans_in_Fill; i++) {
    Get_ODC_separations(1, i); // x-scans
    Get_ODC_separations(2, i); // y-scans
  }

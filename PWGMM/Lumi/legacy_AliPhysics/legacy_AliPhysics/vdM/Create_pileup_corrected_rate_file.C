...
// ratioA/ratioC -> parameters for the pileup correction
{
  ...
  if (scan_type == 1) {
    ...
    sprintf(file_name_new_rate, "../Fill-%d/PileupCorrRate_%s_x_Scan_%d.root",
            g_vdm_Fill, rate_name, scan);
  }
  if (scan_type == 2) {
    sprintf(file_name_rate, "../Fill-%d/BkgdCorrRate_%s_y_Scan_%d.root",
            g_vdm_Fill, rate_name, scan);
    sprintf(file_name_new_rate, "../Fill-%d/PileupCorrRate_%s_y_Scan_%d.root",
            g_vdm_Fill, rate_name, scan);
  }
  ...
  for (Int_t k = 0; k < nIBC; k++) {
    ...
    // compute new rate - loop over steps
                                       ...
                                       Double_t ratioA, Double_t ratioC)
{
  ...
  for (Int_t i = 0; i < g_n_Scans_in_Fill; i++) {
    Create_one_pileup_corrected_rate_file(1, i, rate_name, ratioA,
                                          ratioC); // x-scans
    Create_one_pileup_corrected_rate_file(2, i, rate_name, ratioA,
                                          ratioC); // y-scans
  }

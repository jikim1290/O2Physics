...
Double_t GetXS(Double_t Ax, Double_t Ay, Double_t R00x, Double_t R00y,
               Double_t N1, Double_t N2)
// compute the visible cross sections given the areas, the rates and the
// intensities
{
...

{
  ...
  Double_t DeltaF_2 = (((R00xe * R00xe) / (R00x * R00x * R00x * R00x)) +
                       ((R00ye * R00ye) / (R00y * R00y * R00y * R00y))) /
                      (F * F);
...
Double_t GetHxHyerr(Double_t Ax, Double_t Axe, Double_t Ay, Double_t Aye,
                    Double_t R00x, Double_t R00xe, Double_t R00y,
                    Double_t R00ye)
...
void Find_start_and_end_of_scans()
{
  ...
  if (found_scans == g_n_Scans_in_Fill) {
    ...
    for(Int_t i=0;i<found_scans;i++)
      cout << "   indices are " << g_Idx_Start_Scan_x[i] << " and " <<
    g_Idx_End_Scan_x[i] << endl;
    */
  } else {
    cout << " Number of found x-scans (" << found_scans
         << ") different from expectations: " << g_n_Scans_in_Fill << endl;
    exit(-101);
  }
  ...
  if (found_scans == g_n_Scans_in_Fill) {
    ...
    for(Int_t i=0;i<found_scans;i++)
      cout << "   indices are " << g_Idx_Start_Scan_y[i] << " and " <<
    g_Idx_End_Scan_y[i] << endl;
    */
  } else {
    cout << " Number of found y-scans (" << found_scans
         << ") different from expectations: " << g_n_Scans_in_Fill << endl;
    exit(-102);
  }
}
...
Int_t FindIdxBetweenScans(Int_t scan)
{
  ...
  Int_t idx = (Int_t)((((Double_t)idx_separation_start) +
                       ((Double_t)idx_separation_end)) *
                      0.5);
  return idx;
}
...
Double_t BkgdCorrectedRateError(Double_t r, Double_t re, Double_t f,
                                Double_t fe)
...
Bool_t UseBunchCrossing(Int_t i)
{
  ...
  if (bunches[i] == 942 || bunches[i] == 1022 || bunches[i] == 1142 ||
      bunches[i] == 1655 || bunches[i] == 1695 || bunches[i] == 1735 ||
      bunches[i] == 1953 || bunches[i] == 2033)
    return kFALSE;
  else
    return kTRUE;
}
...
void SetBCBlacklists(bool Verbose = false)
{
  ...
  for (unsigned int a = 0; a < Fills.size(); a++) {
    ...
    if (a >= sizeof(bcBlacklist) / sizeof(bcBlacklist[0])) {
      cout << "SetBCBlacklists:: exceeding pre-defined array size! Stop."
           << endl;
      return;
    }
  }
  ...
  if (Verbose) {
    ...
    for (unsigned int a = 0; a < Fills.size(); a++)
      for (b = bcBlacklist[a].begin(); b != bcBlacklist[a].end(); b++) {
        cout << Form("Preparing blacklist... Fill %i - bc %i", Fills[a], *b)
             << endl;
      }
  }
...
bool OnBCBlacklist(int Fill, int bc, bool Verbose = false)
{
  ...
  if (bcBlacklist[FillIndex].find(bc) != bcBlacklist[FillIndex].end()) {
    if (Verbose)
      cout << Form("BC %i in the Fill %i is on the blacklist!", bc, Fill)
           << endl;
    return true;
  } else
    return false;

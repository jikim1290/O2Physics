...
// 2.- loop over the range of each step and get the correction and its error
{
  ...
  // ** 2.- loop over the range of each step and get the correction and its
  ...
  if (rate_type == 1) {
    ...
    g_vdm_Tree->SetBranchAddress(systBG ? "bv0sys_setv0" : "bv0acc_setv0",
                                 acc_all);
    g_vdm_Tree->SetBranchAddress("bv0tot_setv0", tot_all);
  } else if (rate_type == 2) {
    // g_vdm_Tree->SetBranchAddress("bt0acc_sett0", acc_all);
    g_vdm_Tree->SetBranchAddress(systBG ? "bt0sys_sett0" : "bt0acc_sett0",
                                 acc_all);
  ...
  const char* file_name =
    Form("../Fill-%i/BkgdCorr_%s%s_%s_Scan_%i.root", g_vdm_Fill, strRate,
         systBG ? "SystBG" : "", strScan, scan);
  ...
  {
    if (k % 10 == 0) {
      cout << " Scan " << scan << "," << scan_type << " working on bunch " << k
           << " of " << nIBC << endl
           << flush;
    }
...
void Create_bkgd_correction_file_V0T0(
  Int_t Fill, Int_t rate_type, bool systBG = false) // rate_type: 1=>V0; 2=>T0
{
  cout << " Starting, this will take some time " << endl;
  if (systBG)
    cout << "\n WARNING: you're about to generate an output file for "
            "systematic err check!!!\n\n";

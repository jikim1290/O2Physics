...
void Check_size_of_input_trees(Int_t Fill)
{
  ...
  if ((n_vdm != n_vdm_DDL2) || (n_vdm != n_vdm_BPTX) ||
      (n_vdm_DDL2 != n_vdm_BPTX)) {
    cout << " Entries in " << g_Input_vdm_File << ": " << n_vdm << endl;
    cout << " Entries in " << g_Input_vdm_DDL2_File << ": " << n_vdm_DDL2
         << endl;
    cout << " Entries in " << g_Input_vdm_BPTX_File << ": " << n_vdm_BPTX
         << endl;
  } else {
    cout << " All input trees have the same number of entries: " << n_vdm
         << endl;
  }
}
...
void Check_time_order_of_input_tree(Int_t Fill)
{
  ...
  for (Int_t i = 0; i < n_vdm; i++) {
    ...
    if (time < time_before) {
      not_ordered = kTRUE;
      cout << " Entry " << i << " time_before " << time_before << "  time "
           << time << endl;
    }
    time_before = time;
    if (timerel < timerel_before) {
      cout << " Entry " << i << " timerel_before " << timerel_before
           << "  timerel " << timerel << endl;
      not_ordered = kTRUE;
    }
...
void Checks(Int_t Fill, Int_t opt = 0)
{
  ...
  if (opt == 1)
    Check_time_order_of_input_tree(Fill);
}

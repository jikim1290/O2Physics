...
// scan_type: 1 => x-scan; 2 => y-scan
{
  ...
  GetRate(all, rate_name, Form("IntensityCorr%s", scan == 0 ? "FBCT" : "BPTX"),
          scan, scan_type, bc); // kimc
  ...
  H1->SetTitle(Form("Fill %i, %s, scan %i, bunch pair index %i",
                          Fill, !strcmp(rate_name, "TVX")?"T0":"V0", scan_type,
  bc));
                          */
  H1->GetXaxis()->SetRangeUser(-0.65, 0.65);
  if (Fill == 4937)
    H1->GetYaxis()->SetRangeUser(0.7, 1.25);
  if (Fill == 6012)
    H1->GetYaxis()->SetRangeUser(0.7, 1.30);
  if (Fill == 6864)
    H1->GetYaxis()->SetRangeUser(0.7, 1.35);
  H1->DrawCopy();
  ...
  c1->Print(Form("Fill%i_Ratio_%s_%i.eps", Fill,
                 !strcmp(rate_name, "TVX") ? "T0" : "V0", scan_type));

...
// opt: 1 =>  separations corrected for orbit drift
{
  ...
  if (opt == 0)
    ...
    sprintf(bbd_file_name,
            "../Fill-%d/BBD_FromIvan/ODC-%d/ROOT/bbroot_%d_V0.root", g_vdm_Fill,
            (scan + 1), g_vdm_Fill);
  ...
  if (opt == 0) {
    sprintf(sep_file_name_x, "../Fill-%d/NomBBDSep_x_Scan_%d.root", g_vdm_Fill,
            scan);
    sprintf(sep_file_name_y, "../Fill-%d/NomBBDSep_y_Scan_%d.root", g_vdm_Fill,
            scan);
  } else {
    sprintf(sep_file_name_x, "../Fill-%d/ODCBBDSep_x_Scan_%d.root", g_vdm_Fill,
            scan);
    sprintf(sep_file_name_y, "../Fill-%d/ODCBBDSep_y_Scan_%d.root", g_vdm_Fill,
            scan);
  }
  ...
  for (Int_t k = 0; k < nIBC; k++) {
    ...
    if (nseps_half != n_sep) {
      cout << " number of separations is not compatible! " << nseps << " - "
           << n_sep << " - " << nseps_half << endl;
      exit(-1);
    }
...
// "+Q", "+xi", "-Q", "-xi"
{
  ...
  if (opt == 0)
    ...
    sprintf(bbd_file_name,
            "../Fill-%d/Corr-%d-sys/FBCT-Nom/ROOT/bbroot_%d_V0%s.root", // kimc
            g_vdm_Fill, g_vdm_Fill, g_vdm_Fill, sys_opt);
  else
    sprintf(bbd_file_name,
            "../Fill-%d/Corr-%d-sys/FBCT/ROOT/bbroot_%d_V0%s.root", g_vdm_Fill,

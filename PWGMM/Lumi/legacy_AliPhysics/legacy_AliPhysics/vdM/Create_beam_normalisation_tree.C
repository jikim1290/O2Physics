...
// opt = 0 => fBCT; opt = 1 => BPTX
{
  ...
  if (opt == 0)
    ...
    cout << " Option " << opt << " not recognised. Bye! " << endl;
    return;
  }
  ...
  for (Int_t i = 0; i < nEntries; i++) {
    if (i % 1000 == 0)
      cout << " Working on entry " << i << " of " << nEntries << endl;
    ...
    for (Int_t j = 0; j < 3564; j++) // get total intensity - kimc: this must be
                                     // related to a bunch crossing
    {

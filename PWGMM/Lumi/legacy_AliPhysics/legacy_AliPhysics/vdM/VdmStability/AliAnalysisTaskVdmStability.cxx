...
AliAnalysisTaskVdmStability::~AliAnalysisTaskVdmStability()
{
  ...
  for (Int_t i = 0; i < 25; ++i) {
    ...
    if (emcal_Timing[i]) {
      delete emcal_Timing[i];
    }
    if (muon_Timing[i]) {
      delete muon_Timing[i];
    }
    if (dimuon_Timing[i]) {
      delete dimuon_Timing[i];
    }
  }
}
// defíne output
...
void AliAnalysisTaskVdmStability::UserCreateOutputObjects()
{
  ...
  for (Int_t iCase = 0; iCase < fNSelectionCases; iCase++) {
    v0_H[iCase] = new TH1D(Form("v0_H_%s", selectionCases[iCase].Data()),
                           Form("V0 events, %s", selectionCases[iCase].Data()),
                           fNRuns, -0.5, fNRuns - 0.5);
    t0_H[iCase] = new TH1D(Form("t0_H_%s", selectionCases[iCase].Data()),
...
void AliAnalysisTaskVdmStability::UserExec(Option_t*)
{
  ...
  for (Int_t iCase = 0; iCase < fNSelectionCases; iCase++) {
    ...
    if (IsSelected(iCase)) {
      ...
      if (!badTimeRun)
        v0_Timing[iCase]->Fill(tV0diff, tV0sum);
      if (fIsT0fired) {
        t0_H[iCase]->Fill(binLabel.Data(), 1);
        if (!badTimeRun)
          t0_Timing[iCase]->Fill(tV0diff, tV0sum);
      }
      if (fIsEMCALfired) {
        emcal_H[iCase]->Fill(binLabel.Data(), 1);
        if (!badTimeRun)
          emcal_Timing[iCase]->Fill(tV0diff, tV0sum);
      }
      if (fIsMUfired) {
        muon_H[iCase]->Fill(binLabel.Data(), 1);
        if (!badTimeRun)
          muon_Timing[iCase]->Fill(tV0diff, tV0sum);
      }
      if (fIsDIMUfired) {
        dimuon_H[iCase]->Fill(binLabel.Data(), 1);
        if (!badTimeRun)
          dimuon_Timing[iCase]->Fill(tV0diff, tV0sum);
      }
    }
  }
...
Bool_t AliAnalysisTaskVdmStability::CheckTime(Float_t timeA, Float_t timeC)
{
  ...
  // (tV0sum < 17.5)));
  return (((fV0TimeDiff_min < tV0diff) && (tV0diff < fV0TimeDiff_max)) &&
          ((fV0TimeSum_min < tV0sum) && (tV0sum < fV0TimeSum_max)));
}
                                              ...
                                              Int_t nContCut)
{
  ...
  if (contCut)
    return nCont > nContCut; // if nContCut, return the check
  return kTRUE;
}
...
Bool_t AliAnalysisTaskVdmStability::IsSelected(Int_t nSelection)
{
  ...
  // physics selected
  if (nSelection == 1)
    return (fSelectPhysics);

  // V0 timing cut
  if (nSelection == 2)
    return (fSelectPhysics && fGoodTime);

  // pileup rejection
  if (nSelection == 3)
    return (fSelectPhysics && !fPileupEvent);

  // z-vertex cut (standart: +/-10 nCont > 1)
  if (nSelection == 4)
    return (fSelectPhysics && fIsGoodZ);

  // z-vertex cut (+/-30)
  if (nSelection == 5)
    return (fSelectPhysics && fzCut30);

  // z-vertex cut (+/-10)
  if (nSelection == 6)
    return (fSelectPhysics && fzCut10);

  // z-vertex cut (+/-30) + nCont > 0
  if (nSelection == 7)
    return (fSelectPhysics && fzCut30nCont0);

  // z-vertex cut (+/-10) + nCont > 0
  if (nSelection == 8)
    return (fSelectPhysics && fzCut10nCont0);

  // z-vertex cut (+/-30) + nCont > 1
  if (nSelection == 9)
    return (fSelectPhysics && fzCut30nCont1);

  // z-vertex cut (+/-10) + nCont > 1
  if (nSelection == 10)
    return (fSelectPhysics && fzCut10nCont1);

  // V0 timing cut + pileup rejection
  if (nSelection == 11)
    return (fSelectPhysics && fGoodTime && !fPileupEvent);

  // V0 timing cut + z-veretex cut
  if (nSelection == 12)
    return (fSelectPhysics && fGoodTime && fIsGoodZ);

  // V0 timing cut + pileup rejection + z-vertex cut
  if (nSelection == 13)
    return (fSelectPhysics && fGoodTime && !fPileupEvent && fIsGoodZ);

  // V0 timing cut + pileup rejection + z-vertex cut (+/-30)
  if (nSelection == 14)
    return (fSelectPhysics && fGoodTime && !fPileupEvent && fzCut30);

  // V0 timing cut + pileup rejection + z-vertex cut (+/-10)
  if (nSelection == 15)
    return (fSelectPhysics && fGoodTime && !fPileupEvent && fzCut10);

  // V0 timing cut + pileup rejection + z-vertex cut (+/-30) + nCont > 0
  if (nSelection == 16)
    return (fSelectPhysics && fGoodTime && !fPileupEvent && fzCut30nCont0);

  // V0 timing cut + pileup rejection + z-vertex cut (+/-10) + nCont > 0
  if (nSelection == 17)
    return (fSelectPhysics && fGoodTime && !fPileupEvent && fzCut10nCont0);

  // V0 timing cut + pileup rejection + z-vertex cut (+/-30) + nCont > 1
  if (nSelection == 18)
    return (fSelectPhysics && fGoodTime && !fPileupEvent && fzCut30nCont1);

  // V0 timing cut + pileup rejection + z-vertex cut (+/-10) + nCont > 1
  if (nSelection == 19)
    return (fSelectPhysics && fGoodTime && !fPileupEvent && fzCut10nCont1);

  // V0 timing cut + z-vertex cut (+/-30) + nCont > 0
  if (nSelection == 20)
    return (fSelectPhysics && fGoodTime && fzCut30nCont0);

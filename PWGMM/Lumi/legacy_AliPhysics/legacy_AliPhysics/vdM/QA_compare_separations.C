...
void GetSeparation(Double_t* sep, const char* separation_name, Int_t scan,
                   Int_t scan_type, Int_t bc)
{
  char* file_name = new char[kg_string_size];
  if (scan_type == 1)
    sprintf(file_name, "../Fill-%d/%sSep_x_Scan_%d.root", g_vdm_Fill,
            separation_name, scan);
  if (scan_type == 2)
    sprintf(file_name, "../Fill-%d/%sSep_y_Scan_%d.root", g_vdm_Fill,
            separation_name, scan);
...
// scan_type: 1 => x-scan; 2 => y-scan
{
  ...
  for (Int_t i = 0; i < n_sep; i++) {
    ...
    if (nom[i] > sep_max)
      sep_max = nom[i];
    if (odc_nom[i] < dif_min)
      dif_min = odc_nom[i];
    if (odc_nom[i] > dif_max)
      dif_max = odc_nom[i];
    if (bbd_odc[i] < dif_min)
      dif_min = bbd_odc[i];
    if (bbd_odc[i] > dif_max)
      dif_max = bbd_odc[i];
    if (hys_bbd[i] < dif_min)
      dif_min = hys_bbd[i];
    if (hys_bbd[i] > dif_max)
      dif_max = hys_bbd[i];
  }
  sep_min *= 1.2;
  sep_max *= 1.2;
  ...
  gr_odc_nom->SetMarkerStyle(20);
  gr_odc_nom->SetMarkerColor(1);
  TGraph* gr_bbd_odc = new TGraph(n_sep, nom, bbd_odc);
  gr_bbd_odc->SetMarkerStyle(20);
  gr_bbd_odc->SetMarkerColor(2);
  TGraph* gr_hys_bbd = new TGraph(n_sep, nom, hys_bbd);
  gr_hys_bbd->SetMarkerStyle(20);
  gr_hys_bbd->SetMarkerColor(4);

  // plot graphs
  gStyle->SetOptStat(0);
  gStyle->SetOptTitle(0);
  TCanvas* sep_C = new TCanvas(
    "sep_C", "separation differences versus separation", 600, 400);

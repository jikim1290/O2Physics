...
void Find_separations(Int_t scan_type, Int_t scan_num, Int_t IdxStart,
                      Int_t IdxEnd)
  ...
  for (Int_t j = IdxStart; j < IdxEnd; j++) {
    ...
    if (TMath::Abs(nsep - nsep_old) > small) // new separation
    {
      ...
      if (counter > 2) {
        ...
        //	cout << " slope_change  " << slope_change << "  c " << counter
        //<< endl;
      }
      if ((counter < 3) || (slope_change > 0)) {
        sep_info_tree->Fill();
        cout << scan_type << " New separation = " << sep_array[counter - 1]
             << " starts at " << time_separation_start << " ends at "
             << time_separation_end << " counter " << counter << endl;
      }
...
void Create_nominal_separation_file(Int_t Fill)
{
  ...
  for (Int_t i = 0; i < g_n_Scans_in_Fill; i++) {
    Find_separations(1, i, g_Idx_Start_Scan_x[i],
                     g_Idx_End_Scan_x[i]); // x-scans
    Find_separations(2, i, g_Idx_Start_Scan_y[i],
                     g_Idx_End_Scan_y[i]); // y-scans
  }

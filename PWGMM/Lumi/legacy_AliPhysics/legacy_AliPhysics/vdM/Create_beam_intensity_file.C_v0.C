...
// then compute the intensities at the reference point
{
  ...
  while (i <= end) {
    ...
    i++;
    cout << " time rel " << timerel << " bin center "
         << DCCT1AH->GetBinCenter(2 * i + 1) << endl;
  }
  // average
  cf_dcct_1a /= counter;
  cf_dcct_2a /= counter;
  cf_dcct_1b /= counter;
  cf_dcct_2b /= counter;
  cout << " cf_dcct_1a = " << cf_dcct_1a << " cf_dcct_1b = " << cf_dcct_1b
       << endl
       << " cf_dcct_2a = " << cf_dcct_2a << " cf_dcct_2b = " << cf_dcct_2b
       << endl;

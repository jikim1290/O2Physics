            ...
            const char* sep_type, Int_t fit_type, Int_t scan)
{
  ...
  // histograms
  TH1D* bcc_x_H =
    new TH1D("bcc_x_H", "bcc horizontal scan", nIBC, -0.5, nIBC - 0.5);
  TH1D* bcc_y_H =
    new TH1D("bcc_y_H", "bcc horizontal scan", nIBC, -0.5, nIBC - 0.5);

  // set up the model fit
  TF1* fit_model_x = NULL;
  if (fit_type == 0) {
    fit_model_x =
      new TF1("fit_model", fit_GP2, -1, 1, Get_number_par(fit_type));
  } else if (fit_type == 1) {
    fit_model_x =
      new TF1("fit_model", fit_GP6, -1, 1, Get_number_par(fit_type));
  } else if (fit_type == 2) {
  ...
  for (Int_t i = 0; i < nIBC; i++) {
    ...
    if (chi2_dof_x > 0)
      bcc_x_H->SetBinContent(i + 1, (fit_model_x->Eval(par_x[1])) /
                                      (fit_model_x->Eval(0)));
    if (chi2_dof_y > 0)
      bcc_y_H->SetBinContent(i + 1, (fit_model_y->Eval(par_y[1])) /
                                      (fit_model_y->Eval(0)));
  }

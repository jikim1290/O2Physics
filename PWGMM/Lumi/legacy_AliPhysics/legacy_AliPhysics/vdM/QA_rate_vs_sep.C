...
// scan_type: 1 => x-scan; 2 => y-scan
{
  ...
  // --> separation file
  if (scan_type == 1)
    sprintf(file_name, "../Fill-%d/%sSep_x_Scan_%d.root", g_vdm_Fill, sep_type,
            scan);
  if (scan_type == 2)
    sprintf(file_name, "../Fill-%d/%sSep_y_Scan_%d.root", g_vdm_Fill, sep_type,
            scan);
  ...
  rvss_C->Print(Form("c2a_QArate_Fill%i_%s_%s_%s_scanT%i_scan%i_bc%i.%s", Fill,
                     rate_name, rate_type, sep_type, scan_type, scan, bc,
                     FFormat));
}

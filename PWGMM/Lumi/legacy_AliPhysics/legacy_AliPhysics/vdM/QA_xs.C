           ...
           Int_t scan)
{
  ...
  // plot histo
  const char* TYPE =
    Form("Fill%i_%s_%s_%s_%s_fit%i_scan%i", Fill, sep_type, intensity_type,
         rate_name, rate_type, fit_type, scan); // kimc

  if (!strcmp(rate_name, "VBAandVBC")) {
    yMin = 54;
    yMax = 58;
  } else if (!strcmp(rate_name, "TVX")) {
    yMin = 26;
    yMax = 30;
  }

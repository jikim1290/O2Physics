...
void GetHistogram(Int_t Fill, Int_t scan)
{
  ...
  TH1D* fbct_beam1_H =
    new TH1D("fbct_beam1_H", "fbct_beam1", nIBCUsed, -0.5, nIBCUsed - 0.5);
  TH1D* fbct_beam2_H =
    new TH1D("fbct_beam2_H", "fbct_beam2", nIBCUsed, -0.5, nIBCUsed - 0.5);
  TH1D* bptx_beam1_H =
    new TH1D("bptx_beam1_H", "fbct_beam1", nIBCUsed, -0.5, nIBCUsed - 0.5);
  TH1D* bptx_beam2_H =
    new TH1D("bptx_beam2_H", "bptx_beam2", nIBCUsed, -0.5, nIBCUsed - 0.5);

  Int_t bin = 1;
  for (Int_t i = 0; i < nIBC; i++) {
    if (!UseBunchCrossing(i))
      continue;
    fbct_beam1_H->SetBinContent(bin,
                                fbct_cf_dcct_1 * fbct_bunch_intensity_1[i]);
    fbct_beam2_H->SetBinContent(bin,
                                fbct_cf_dcct_2 * fbct_bunch_intensity_2[i]);
    bptx_beam1_H->SetBinContent(bin,
                                bptx_cf_dcct_1 * bptx_bunch_intensity_1[i]);
    bptx_beam2_H->SetBinContent(bin,
                                bptx_cf_dcct_2 * bptx_bunch_intensity_2[i]);
    bin++;
  }
  ...
  BeutifyTH1(ratio_beam1_H, Form("Scan %d Beam 1;BC;Ratio:BPTX/fBCT", scan), 2,
             1, 20, 1, 1.2);
  BeutifyTH1(ratio_beam2_H, Form("Scan %d Beam 2;BC;Ratio:BPTX/fBCT", scan), 2,
             1, 20, 1, 1.2);

  switch (scan) {
    case 0:
      ratio_H[0] = ratio_beam1_H;
      ratio_H[1] = ratio_beam2_H;
      break;
    case 1:
      ratio_H[2] = ratio_beam1_H;
...
void QA_fbct_bptx(Int_t Fill, Bool_t save = kFALSE)
{
  ...
  graphPad->cd(1);
  ratio_H[0]->SetMaximum(1.05);
  ratio_H[0]->SetMinimum(0.95);
  ratio_H[0]->Fit("pol0");
  ratio_H[0]->Draw("p");
  graphPad->cd(3);
  ratio_H[1]->SetMaximum(1.05);
  ratio_H[1]->SetMinimum(0.95);
  ratio_H[1]->Fit("pol0");
  ratio_H[1]->Draw("p");

  graphPad->cd(2);
  ratio_H[2]->SetMaximum(1.05);
  ratio_H[2]->SetMinimum(0.95);
  ratio_H[2]->Fit("pol0");
  ratio_H[2]->Draw("p");

  graphPad->cd(4);
  ratio_H[3]->SetMaximum(1.05);
  ratio_H[3]->SetMinimum(0.95);
  ratio_H[3]->Draw("p");
  ratio_H[3]->Fit("pol0");

                   ...
                   Int_t fit_type, Int_t scan)
{
  ...
  for (Int_t i = 0; i < nIBC; i++) {
    ...
    if (chi2_dof_x > 0 && chi2_dof_y > 0) {
      // fill histograms
      Double_t hxhy =
        GetHxHy(area_x[0], area_y[0], rate_zero_x[0], rate_zero_y[0]);
      Double_t hxhy_err =
        GetHxHyerr(area_x[0], area_x[1], area_y[0], area_y[1], rate_zero_x[0],
                   rate_zero_x[1], rate_zero_y[0], rate_zero_y[1]);
      hxhy_H->SetBinContent(i + 1, hxhy);
                  ...
                  Bool_t save = kTRUE)
{
  ...
  // Set histograms
  TH1D* hxhy_V0_0 = GetHistogram("VBAandVBC", rate_type, sep_type,
                                 intensity_type, fit_type, 0);
  TH1D* hxhy_V0_1 = GetHistogram("VBAandVBC", rate_type, sep_type,
                                 intensity_type, fit_type, 1);
  TH1D* hxhy_T0_0 =
    GetHistogram("TVX", rate_type, sep_type, intensity_type, fit_type, 0);
  TH1D* hxhy_T0_1 =
    GetHistogram("TVX", rate_type, sep_type, intensity_type, fit_type, 1);
  ...
  BeutifyTH1(ratio[0], Form(";Bunch pair ID; %s", name.Data()), 2, 1, 20, 1,
             1.2);
  BeutifyTH1(ratio[1], Form(";Bunch pair ID; %s", name.Data()), 2, 1, 20, 1,
             1.2);
  ...
  for (int a = 0; a < 2; a++) {
    ...
    L3->AddEntry((TObject*)0,
                 Form("%5.2f/%i", F1->GetChisquare(), F1->GetNDF()), "");
    L3->AddEntry((TObject*)0, "Mean", "");
    L3->AddEntry(
      (TObject*)0,
      Form("%5.3f #pm %4.3f", F1->GetParameter(0), F1->GetParError(0)), "");
    L3->Draw();
  }

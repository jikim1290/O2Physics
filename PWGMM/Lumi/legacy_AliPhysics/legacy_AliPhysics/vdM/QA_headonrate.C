                   ...
                   Int_t fit_type, Int_t scan)
{
  ...
  BeutifyTH1(ratio_H,
             Form("Scan %d;BC;Ratio: #frac{R^{x}(0,0)}{R^{y}(0,0)}", scan), 2,
             1, 20, 1, 1.2);

  return ratio_H;
}

void QA_headonrate(Int_t Fill, const char* rate_name, const char* rate_type,
                   const char* sep_type, const char* intensity_type,
                   Int_t fit_type, Bool_t save = kTRUE)
{
  ...
  TH1D* ratio_0 = GetHistogram(Fill, rate_name, rate_type, sep_type,
                               intensity_type, fit_type, 0);
  TH1D* ratio_1 = GetHistogram(Fill, rate_name, rate_type, sep_type,
  ...
  ratio_0->SetMaximum(1.04);
  ratio_0->SetMinimum(0.96);
  ratio_0->Fit("pol0");
  ratio_0->Draw("p");

  graphPad->cd(2);
  ratio_1->SetMaximum(1.04);
  ratio_1->SetMinimum(0.96);
  ratio_1->Fit("pol0");
  ratio_1->Draw("p");
  ...
  if (save) {
    const char* TYPE =
      Form("Fill%i_%s_%s_%s_%s_fit%i", Fill, sep_type, intensity_type,
           rate_name, rate_type, fit_type); // kimc
    TString plotName = Form("c3c_headonrate_%s.%s", TYPE, FFormat);
    // TString plotName =
    // Form("../Fill-%d/Plots/%s_headonrate.pdf",Fill,rate_name);

...
void Create_BBD_separation_file(Int_t Fill, Int_t scan, const char* rate_name,
                                ...
                                const char* intensity_type)
{
  ...
  sprintf(file_name, "../Fill-%d/BBD_%s_%s_%s_scan_%d.txt", Fill, rate_name,
          intensity_type, sep_type, (scan + 1));
  ofstream bbd_file;
  ...
  bbd_file << "2) Betax*, Betay* [m,m] :" << kg_beta_x << "," << kg_beta_y
           << endl;
  ...
  sprintf(sepx_file_name, "../Fill-%d/%sSep_x_Scan_%d.root", g_vdm_Fill,
          sep_type, scan);
  ...
  sprintf(sepy_file_name, "../Fill-%d/%sSep_y_Scan_%d.root", g_vdm_Fill,
          sep_type, scan);
  ...
  for (Int_t k = 0; k < nIBC; k++) // loop over bunches
  {
    ...
    bbd_file << "6) BC Sx,Sy,Np1,Np2 [um,um,prot,prot] :" << bunches[k] << ","
             << (1000 * area_x[0] /
                 (rate_zero_x[0] * TMath::Sqrt(TMath::TwoPi())))
             << ","
             << (1000 * area_y[0] /
                 (rate_zero_y[0] * TMath::Sqrt(TMath::TwoPi())))

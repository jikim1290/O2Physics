...
// then loop over all bunch crossings and for each of them make a fit
{
  ...
  if (scan_type == 2)
    sprintf(rate_file_name, "../Fill-%d/%sRate_%s_y_Scan_%d.root", g_vdm_Fill,
            rate_type, rate_name, scan);
  ...
  if (scan_type == 1)
    sprintf(sep_file_name, "../Fill-%d/%sSep_x_Scan_%d.root", g_vdm_Fill,
            sep_type, scan);
  if (scan_type == 2)
    sprintf(sep_file_name, "../Fill-%d/%sSep_y_Scan_%d.root", g_vdm_Fill,
            sep_type, scan);
  ...
  for (Int_t k = 0; k < nIBC; k++) // loop over bunches
  {
    ...
    if (scan_type == 2)
      cName3 = Form("%s_y", cName2);
    chi2_dof = Fit_rate_separation(n_sep, sep, rate, rate_error, fit_type, area,
                                   rate_zero, par, par_err, cName3);

    /*
    chi2_dof = Fit_rate_separation_minuit(
                    n_sep, sep, rate, rate_error, fit_type, area, rate_zero,
    par, par_err, scan, scan_type, bc
                    );
                    */
                      ...
                      const char* sep_type, Int_t fit_type)
{
  ...
  for (Int_t i = 0; i < g_n_Scans_in_Fill; i++) {
    Compute_RateIntegral(Fill, 1, i, rate_name, rate_type, sep_type,
                         fit_type); // x-scans
    Compute_RateIntegral(Fill, 2, i, rate_name, rate_type, sep_type,
                         fit_type); // y-scans
  }

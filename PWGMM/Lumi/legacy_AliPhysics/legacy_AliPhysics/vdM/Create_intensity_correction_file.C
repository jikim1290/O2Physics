...
void Compute_intensity_correction(Int_t Fill, Int_t scan_type, Int_t scan,
                                  Int_t intensity_type)
...
// 3.- loop over the range of each step and get the correction and its error
{
  ...
  if (scan_type == 1) {
    if (intensity_type == 1) {
      ...
      sprintf(file_name, "../Fill-%d/FBCTCorr_x_Scan_%d.root", g_vdm_Fill,
              scan);
    }
  } else if (scan_type == 2) {
    if (intensity_type == 1) {
      sprintf(file_name, "../Fill-%d/BPTXCorr_y_Scan_%d.root", g_vdm_Fill,
              scan);
    } else if (intensity_type == 2) {
      sprintf(file_name, "../Fill-%d/FBCTCorr_y_Scan_%d.root", g_vdm_Fill,
              scan);
    }
  }
  ...
  {
    if (k % 10 == 0)
      cout << Form("Scan %i, ScanType %i, Bunch %2i/%i\n", scan, scan_type, k,
                   nIBC)
           << flush;
    ...
    for (Int_t i = 0; i < n_separations; i++) // loop over steps
    {
      ...
      for (Int_t j = idx_start[i]; j <= idx_end[i]; j++) {
        ...
        bi1_2 += ((cf_dcct_1 * bunch1[idx1] / gBeamA) *
                  (cf_dcct_1 * bunch1[idx1] / gBeamA));
        bi2_2 += ((cf_dcct_2 * bunch2[idx2] / gBeamB) *
                  (cf_dcct_2 * bunch2[idx2] / gBeamB));
        ni += 1.0;

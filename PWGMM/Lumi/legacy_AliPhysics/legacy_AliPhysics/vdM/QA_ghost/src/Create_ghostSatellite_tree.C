...
Bool_t CheckVariableName(TString name, Int_t& variable)
{
  ...
  if (!temp.CompareTo("Q_ghost_bunched")) {
    useVariable = kTRUE;
    variable = 1;
  }
  if (!temp.CompareTo("Q_sat_bunched")) {
    useVariable = kTRUE;
    variable = 2;
  }
  if (!temp.CompareTo("sigma_ghost_bunched")) {
    useVariable = kTRUE;
    variable = 3;
  }
  if (!temp.CompareTo("sigma_sat_bunched")) {
    useVariable = kTRUE;
    variable = 4;
  }
  if (!temp.CompareTo("sigma_ghost_bunched_stat")) {
    useVariable = kTRUE;
    variable = 5;
  }
  if (!temp.CompareTo("sigma_ghost_bunched_syst")) {
    useVariable = kTRUE;
    variable = 6;
  }
  if (!temp.CompareTo("sigma_sat_bunched_stat")) {
    useVariable = kTRUE;
    variable = 7;
  }
  if (!temp.CompareTo("sigma_sat_bunched_syst")) {
    useVariable = kTRUE;
    variable = 8;
  }

  if (useVariable)
    cout << "Use variable " << temp.Data() << endl;
  return useVariable;
}
...
Bool_t InsideScanPeriod(UInt_t time)
{
  ...
  if (time >= gScan0StartX && time <= gScan0EndX)
    isInside = kTRUE;
  if (time >= gScan0StartY && time <= gScan0EndY)
    isInside = kTRUE;
  if (time >= gScan1StartX && time <= gScan1EndX)
    isInside = kTRUE;
  if (time >= gScan1StartY && time <= gScan1EndY)
    isInside = kTRUE;
...
// opt: 2 => beam 2
{
  ...
  if (opt == 0)
    ...
    sprintf(data_file_name, "../Fill-%d/ghostCharge/%d_B1.SYS_B.CSV", Fill,
            Fill);
  else
    sprintf(data_file_name, "../Fill-%d/ghostCharge/%d_B2.SYS_A.CSV", Fill,
            Fill);
  ...
  while (!line.ReadLine(data_file).eof()) {
    ...
    } else if (line.Contains("DataType") || line.Contains("Unit") ||
               line.Contains("Length") || line.Contains("Description") ||
  ...
  ghost_tree->Branch("sigma_ghost_time", &sigma_ghost_time,
                     "sigma_ghost_time/i");
  ghost_tree->Branch("sigma_ghost_value", &sigma_ghost_value,
                     "sigma_ghost_value/D");
  ghost_tree->Branch("sigma_ghost_value_stat", &sigma_ghost_value_stat,
                     "sigma_ghost_value_stat/D");
  ghost_tree->Branch("sigma_ghost_value_syst", &sigma_ghost_value_syst,
                     "sigma_ghost_value_syst/D");
  ...
  satellite_tree->Branch("sigma_sat_value", &sigma_sat_value,
                         "sigma_sat_value/D");
  satellite_tree->Branch("sigma_sat_value_stat", &sigma_sat_value_stat,
                         "sigma_sat_value_stat/D");
  satellite_tree->Branch("sigma_sat_value_syst", &sigma_sat_value_syst,
                         "sigma_sat_value_syst/D");
...
void Create_ghostSatellite_tree(Int_t Fill)
{
  ...
  cout << "Determining the start and end times of each scan, may take some time"
       << endl;

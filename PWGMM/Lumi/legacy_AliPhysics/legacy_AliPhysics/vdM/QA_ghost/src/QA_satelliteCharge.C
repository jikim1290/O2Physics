...

{
  ...
  // fill graph
  TGraphErrors* sat_gr =
    new TGraphErrors(ng, timeArray, satArray, NULL, satErrArray);
  ...
  Double_t time_max = 0;
  for (Int_t i = 0; i < ng; i++) {
    if (timeArray[i] < time_min)
      time_min = timeArray[i];
    if (timeArray[i] > time_max)
      time_max = timeArray[i];
  }
  time_min -= 1e3;
  time_max += 1e3;
  ...
  if (opt == 2)
    device = "B2";
  frame->SetTitle(Form(
    "Fraction of beam in Satellite Bucket (%s);timestamp ; satellite charge",
    device.Data()));
  ...
  if (save) {
    ...
    TString plotName = "";
    if (opt == 0)
      plotName = Form("../Fill-%d/ghostCharge/satelliteCharge_B1_A_%s.pdf",
                      Fill, unc.Data());
    if (opt == 1)
      plotName = Form("../Fill-%d/ghostCharge/satelliteCharge_B1_B_%s.pdf",
                      Fill, unc.Data());
    if (opt == 2)
      plotName = Form("../Fill-%d/ghostCharge/satelliteCharge_B2_A_%s.pdf",
                      Fill, unc.Data());

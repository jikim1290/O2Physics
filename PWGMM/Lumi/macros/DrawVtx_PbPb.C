{
  ...
  TFile* ftot = new TFile(Form("../data/PbPb_Run2conv/AnalysisResults_%s.root", fname), "read");
  ...
  TPaletteAxis* paletteX = (TPaletteAxis*)hTime_VtxX->GetListOfFunctions()->FindObject("palette");
  ...
  const int nstage = 3;
  double StepLength[nstage] = {
    0.0021079, 0.0042159, 0.003243};

  const int nsteps[nstage] = {
    6, 3, 4};

  double trange_lower_x[3][6] = {
    {580, 680, 760, 840, 920, 1010},
    {1120, 1220, 1300, -1, -1, -1},
    {1400, 1500, 1600, 1680, -1, -1}};
  double trange_upper_x[3][6] = {
    {620, 720, 800, 880, 960, 1050},
  ...
  for (int s = 0; s < nstage; s++) {
    ...
    for (int i = 0; i < nsteps[s]; i++) {
      ...
      line_xl_1st[s][i] = new TLine(1e3 * trange_lower_x[s][i], 0.08, 1e3 * trange_lower_x[s][i], 0.11);
      line_xu_1st[s][i] = new TLine(1e3 * trange_upper_x[s][i], 0.08, 1e3 * trange_upper_x[s][i], 0.11);
    ...
    c->SaveAs(Form("../figures_%s/vertexX_297325_full_zoom%d.pdf", fname, s + 1));
  }
  ...
  TPaletteAxis* paletteY = (TPaletteAxis*)hTime_VtxY->GetListOfFunctions()->FindObject("palette");
  paletteY->SetY1NDC(0.2);
  c->Update();
  hTime_VtxY->Draw("colz");
  c->SaveAs(Form("../figures_%s/vertexY_297325_full.pdf", fname));

  double trange_lower_y[3][6] = {
    {1820, 1920, 2000, 2100, 2180, 2260},
    {2360, 2460, 2540, -1, -1, -1},
    {2640, 2740, 2840, 2920, -1, -1}};
  double trange_upper_y[3][6] = {
    {1860, 1960, 2040, 2140, 2220, 2300},
  ...
  for (int s = 0; s < nstage; s++) {
    ...
    for (int i = 0; i < nsteps[s]; i++) {
      ...
      line_yl_1st[s][i] = new TLine(1e3 * trange_lower_y[s][i], 0.355, 1e3 * trange_lower_y[s][i], 0.385);
      line_yu_1st[s][i] = new TLine(1e3 * trange_upper_y[s][i], 0.355, 1e3 * trange_upper_y[s][i], 0.385);
    ...
    c->SaveAs(Form("../figures_%s/vertexY_297325_full_zoom%d.pdf", fname, s + 1));
  }
  ...
  for (int s = 0; s < nstage; s++) {
    for (int i = 0; i < nsteps[s]; i++) {
      hVtxX_projected[s][i] = (TH1D*)hTime_VtxX->ProjectionY(Form("hVtxX_projected_%d_%d", s, i),
      ...
      hVtxY_projected[s][i] = (TH1D*)hTime_VtxY->ProjectionY(Form("hVtxY_projected_%d_%d", s, i),
  ...
  for (int s = 0; s < nstage; s++) {
    ...
    legx->AddEntry(flinear, Form("p_{0} + p_{1}*x, #chi^{2} = %.1lf", flinear->GetChisquare()), "l");
    legx->AddEntry((TObject*)0, Form("p_{0} = %.3lf #pm %.3lf", flinear->GetParameter(0), flinear->GetParError(0)), "");
    legx->AddEntry((TObject*)0, Form("p_{1} = %.3lf #pm %.3lf", flinear->GetParameter(1), flinear->GetParError(1)), "");
    ...
    legy->AddEntry(flinear, Form("p_{0} + p_{1}*x, #chi^{2} = %.1lf", flinear->GetChisquare()), "l");
    legy->AddEntry((TObject*)0, Form("p_{0} = %.3lf #pm %.3lf", flinear->GetParameter(0), flinear->GetParError(0)), "");
    legy->AddEntry((TObject*)0, Form("p_{1} = %.3lf #pm %.3lf", flinear->GetParameter(1), flinear->GetParError(1)), "");
  ...
  for (int s = 0; s < nstage; s++) {
    ...
    gY[s]->Write();
    for (int i = 0; i < nsteps[s]; i++) {
      hVtxX_projected[s][i]->SetName(Form("hVtxX_projected_%s_%d_%d", fname, s, i));
      hVtxY_projected[s][i]->SetName(Form("hVtxY_projected_%s_%d_%d", fname, s, i));
